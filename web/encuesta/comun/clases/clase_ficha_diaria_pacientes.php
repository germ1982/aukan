<?
         ...
         function clase_ficha_diaria_pacientes($id)
         {
      	 ...
      	 function asigna($arreglo)
      	 {
      	 	...
      	 	$this->solicito_cirugia=$arreglo['solicito_cirugia'];
                ...
                $this->actividad_fisica = $arreglo['actividad_fisica'];
      	 }
      ...
      function guardar()
      {
          $bd = new baseDatos();
      	  ...
      	  if ($this->id==0 || $this->id=='' ) {
      	      if ($bd->select("INSERT INTO ficha_diaria_pacientes(idprofesional,idpaciente,fecha,hora,motivo_consulta,examen_fisico,estudios_especiales,diagnostico,tratamiento,observaciones,peso,talla,perimetro,proxima_visita,otro_dato,codigo_estudio,codigo_diagnostico,codigo_tratamiento,solicito_cirugia,interconsulta,fq,tac_rnm,tratamiento_solicitado,cirugia,nada,rx,coloco_yeso,realizo_infiltracion,conceptId,programada,id_encuesta,actividad_fisica) VALUES('".$this->idprofesional."','".$this->idpaciente."','".$this->fecha."','".$this->hora."','".$bd->parser($this->motivo_consulta)."','".$bd->parser($this->examen_fisico)."','".$bd->parser($this->estudios_especiales)."','".$this->diagnostico."','".$bd->parser($this->tratamiento)."','".$bd->parser($this->observaciones)."','".$this->peso."','".$this->talla."','".$this->perimetro."','".$this->proxima_visita."','".$bd->parser($this->otro_dato)."','".$this->codigo_estudio."','".$this->codigo_diagnostico."','".$this->codigo_tratamiento."','".$this->solicito_cirugia."','" . $this->interconsulta . "','" . $this->fq . "','" . $this->tac_rnm . "','" . $this->tratamiento_solicitado . "','" . $this->cirugia . "','" . $this->nada . "','" . $this->rx . "','".$this->coloco_yeso."','".$this->realizo_infiltracion."','".$this->conceptId."','".$this->programada."','".$this->id_encuesta."','".$this->actividad_fisica."')"))
      	      {
    ...
    function actividad_fisica()
          ...
          function actividad_fisica_asigna($campo)
          {
               $this->actividad_fisica=$campo;
          }
