	...
	class clase_indicaciones
	{
		...
		function indicaciones_realizadas($idepisodio,$fdesde,$fhasta,$fecha_egreso,$idindicacion_sala)
		{
			...
			$bd->select("SELECT iddroga, idpresentacion, idcama, SUM( indicacion_horas_suministrada.cantidad ) as cantidad,
						...
						GROUP BY iddroga, idpresentacion, idcama");
		...
		function indicaciones_indicadas($fdesde,$fhasta,$idlugar,$paciente_estado,$idpaciente)
		{
			//paciente_estado es 0 si es para pacientes que estan internados, 1 para pacientes que se fuero, 2 para todo los pacientes
			$bd = new baseDatos();
			$bd->Conectarse();
			$base = new baseDatos();
			$base->Conectarse();
			if ($idpaciente != '') $where_paciente = " AND idpaciente=$idpaciente ";
			if ($paciente_estado == 0) $where_episodios = " AND (fecha_egreso = '0000-00-00' OR fecha_egreso = '') ";
			if ($paciente_estado == 1) $where_episodios = " AND (fecha_egreso <> '0000-00-00' AND fecha_egreso <> '') ";
			
			$where = " AND indicaciones_medicas.fecha>='".fechaBase($fdesde)."' AND indicaciones_medicas.fecha<='".fechaBase($fhasta)."' ";
			$bd->select("SELECT idepisodio,idpaciente FROM episodios WHERE idlugar=$idlugar $where_paciente $where_episodios GROUP BY idepisodio");
			
			$pro = new clase_listar();
			while ($arreglo_episodio = $bd->registro())
		    {		    	  
		        $base->select("SELECT iddroga,idpresentacion,unidad_medida,SUM((dosis*(24/intervalo))) as dosis 
                  		    FROM indicaciones_medicas LEFT JOIN indicaciones_sala ON 
                  			(indicaciones_medicas.id=indicaciones_sala.idindicacionsala) 
                  			WHERE indicaciones_medicas.idepisodio=".$arreglo_episodio['idepisodio']." $where  
                  			GROUP BY iddroga,idpresentacion,unidad_medida");
		    	...
		    	for($i=0;$i<=$base->numero_filas();$i++) 
		    	{
		    		...
		    		if ($fila['iddroga'] != 0 && $fila['iddroga'] !='')
		    		{
		    			$fila['idpaciente'] = $arreglo_episodio['idpaciente'];
		    			$fila['idepisodio'] = $arreglo_episodio['idepisodio'];
		    		    $pro->introducirElemento($fila);
		    		} 
		    	}		    		        
		    }
