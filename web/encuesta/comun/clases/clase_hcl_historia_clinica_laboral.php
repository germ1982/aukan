...
class clase_hcl_historia_clinica_laboral {
    ...
    function guardar() {
        ...
        if ($this->numero_transaccion == 0 || $this->numero_transaccion == '') {
            if ($bd->select("INSERT INTO hcl_historia_clinica_laboral(idprofesional,idpaciente,fecha,hora,descripcion,proximavisita,es_ultima_visita,numero,interconsulta,fq,tac_rnm,tratamiento,cirugia,nada,rx,coloco_yeso,realizo_infiltracion) VALUES('" . $this->idprofesional . "','" . $this->idpaciente . "','" . $this->fecha . "','" . $this->hora . "','" . $bd->parser($this->descripcion) . "','" . $this->proximavisita . "','" . $this->es_ultima_visita . "','" . $this->numero . "','" . $this->interconsulta . "','" . $this->fq . "','" . $this->tac_rnm . "','" . $this->tratamiento . "','" . $this->cirugia . "','" . $this->nada . "','" . $this->rx . "','".$this->coloco_yeso."','".$this->realizo_infiltracion."')")) {
                $this->numero_transaccion = $bd->ultimo_id();
                return 1;
            } else
                return 0;
        }else {
            if ($bd->select("UPDATE hcl_historia_clinica_laboral SET coloco_yeso='".$this->coloco_yeso."',realizo_infiltracion='".$this->realizo_infiltracion."',idprofesional='" . $this->idprofesional . "',idpaciente='" . $this->idpaciente . "',fecha='" . $this->fecha . "',hora='" . $this->hora . "',descripcion='" . $bd->parser($this->descripcion) . "',proximavisita='" . $this->proximavisita . "',es_ultima_visita='" . $this->es_ultima_visita . "',numero='" . $this->numero . "',interconsulta='" . $this->interconsulta . "',fq='" . $this->fq . "',tac_rnm='" . $this->tac_rnm . "',tratamiento='" . $this->tratamiento . "',cirugia='" . $this->cirugia . "',nada='" . $this->nada . "',rx='" . $this->rx . "' WHERE numero_transaccion='" . $this->numero_transaccion . "'")) {
