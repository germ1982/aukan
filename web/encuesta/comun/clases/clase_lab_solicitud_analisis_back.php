      ...
      function guardar()
      {
          $bd = new baseDatos();
      	  ...
      	  if ($this->id==0 || $this->id=='' ) {
      	      if ($bd->select("INSERT INTO lab_solicitud_analisis(fecha,cli_paciente_id,cli_profesional_solicitante_id,resumen_clinico,observaciones,baja_fecha,idosocial) VALUES('".$this->fecha."','".$this->cli_paciente_id."','".$this->cli_profesional_solicitante_id."','".$this->resumen_clinico."','".$this->observaciones."','".$this->baja_fecha."','".$this->idosocial."')"))
      	      {
      	  ...
      	  { 
      	        if ($bd->select("UPDATE lab_solicitud_analisis SET fecha='".$this->fecha."',cli_profesional_solicitante_id='".$this->cli_profesional_solicitante_id."',resumen_clinico='".$this->resumen_clinico."',observaciones='".$this->observaciones."',idosocial='".$this->idosocial."' WHERE id='".$this->id."'"))
      	        {
	      ...
	      function foranea_cli_paciente_id($cli_paciente_id)
		  {
				$bd = new baseDatos();
				$bd->Conectarse();		    
				$bd->select("SELECT * FROM lab_solicitud_analisis WHERE cli_paciente_id=$cli_paciente_id ORDER BY fecha DESC");				
	      ...
	      function foranea_cli_profesional_solicitante_id($cli_profesional_solicitante_id)
		  {
				$bd = new baseDatos();
				$bd->Conectarse();		    
				$bd->select("SELECT * FROM lab_solicitud_analisis WHERE cli_profesional_solicitante_id=$cli_profesional_solicitante_id");				
				$pro = new clase_listar();			
		  ...
		  function foranea_cli_paciente_idosocial($cli_paciente_id,$idosocial)
		  {
				...
				$bd->select("SELECT * FROM lab_solicitud_analisis WHERE 
                                            idosocial=$idosocial AND cli_paciente_id=$cli_paciente_id ORDER BY fecha DESC");				
				$pro = new clase_listar();			
