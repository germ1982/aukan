<?
	class clase_pacientes 
	{
		...
		function clase_pacientes($id)
		{
			$bd = new baseDatos();
			$bd->Conectarse();
			$bd->set_names();
			$bd->select("SELECT * FROM pacientes WHERE idpaciente=$id");
		...
		function federateCliPaciente($idpaciente,$paciente_federadoid)
		{
			$bd = new baseDatos();
			$bd->Conectarse();
			if ($bd->select("UPDATE pacientes SET paciente_federadoid=$paciente_federadoid WHERE idpaciente=$idpaciente"))
			    return 1;
			else 
			    return 0;			
		}
		function verificarExistencia($numero_documento, $nombres, $apellidos, $fecha_nacimiento, $sexo)
		{
			$bd = new baseDatos();
			$bd->Conectarse();
			$bd->select("SELECT * FROM pacientes 
		...
		function idpacienteFederadoLocal($idpaciente_federado)
		{
			...
			$bd->Conectarse();
			$bd->set_names();
			$bd->select("SELECT * FROM pacientes WHERE paciente_federadoid=$idpaciente_federado");
			$nombre_pac = $bd->registro();
		...
		function guardar()
		{
			...
			if ($bd->select("UPDATE pacientes SET hora_nacimiento='".$this->hora_nacimiento."',lugar_fisico_nacimiento='".$this->lugar_fisico_nacimiento."' WHERE idpaciente=".$this->idpaciente))
			    return 1;
			else 
			    return 0;
		}
		function guardar_grupo_factor_madre()
		{
			...
			$bd->Conectarse();
			//por ahora solo hago que actualice los campos que necesito que son hora_nacimiento y lugar_fisico_nacimiento
			if ($bd->select("UPDATE pacientes SET grupo_factor_madre='".$this->grupo_factor_madre."' WHERE idpaciente=".$this->idpaciente))
			    return 1;
			else 
			    return 0;
		}
		function buscar_por_documento($dni)
		{
			$bd = new baseDatos();
			$bd->Conectarse();
			$bd->select("SELECT * FROM pacientes WHERE documento='$dni'");
