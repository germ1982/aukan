                      ...
                      if ($que_es !=0)
                          $bd->select("SELECT * 
                                     FROM pedidos_estudio LEFT JOIN pedidos_estudio_detalle USING(idpedido_estudio) 
                                          LEFT JOIN resultados_estudio USING (idpedido_estudio_detalle) 
                                          LEFT JOIN nomenclador USING (idnomenclador)
                                     WHERE idpaciente=$idpaciente AND pedidos_estudio.fecha='".$fecha."' AND que_es=$que_es");
                      else
                      {
                          if ($codigo_contenedor != 0)
                          {
                              $bd->select("SELECT * 
                                     FROM pedidos_estudio LEFT JOIN pedidos_estudio_detalle USING(idpedido_estudio) 
                                          LEFT JOIN resultados_estudio USING (idpedido_estudio_detalle) 
                                          LEFT JOIN nomenclador USING (idnomenclador)
                                     WHERE idpaciente=$idpaciente AND pedidos_estudio.fecha='".$fecha."' AND codigo_contenedor='$codigo_contenedor'");
