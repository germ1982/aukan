<?
      ...
      var $arreglo_foraneo_cod_nom='';
         ...
         function clase_hcl_nomeclador($id)
         {
      	     $bd = new baseDatos();
      	     $bd->Conectarse();
      	     $bd->select("SELECT * FROM hcl_nomeclador WHERE id=$id");
      	     $arreglo=$bd->registro();
      	     $this->id=$arreglo['id'];
      ...
      function guardar()
      {
          $bd = new baseDatos();
      	  ...
      	  if ($this->id==0 || $this->id=='' ) {
      	      if ($bd->select("INSERT INTO hcl_nomeclador(cod_nom,idprofesional,idpaciente,fecha,hora,cantidad) VALUES('".$this->cod_nom."','".$this->idprofesional."','".$this->idpaciente."','".$this->fecha."','".$this->hora."','".$this->cantidad."')"))
      	      {
      	  ...
      	  { 
      	        if ($bd->select("UPDATE hcl_nomeclador SET cod_nom='".$this->cod_nom."',idprofesional='".$this->idprofesional."',idpaciente='".$this->idpaciente."',fecha='".$this->fecha."',hora='".$this->hora."',cantidad='".$this->cantidad."' WHERE id='".$this->id."'"))
      	        {
	      ...
	      function foranea_cod_nom($cod_nom)
		  {
				$bd = new baseDatos();
				$bd->Conectarse();		    
				$bd->select("SELECT * FROM hcl_nomeclador WHERE cod_nom=$cod_nom");				
	      ...
	      function foranea_idprofesional($idprofesional)
		  {
				$bd = new baseDatos();
				$bd->Conectarse();		    
				$bd->select("SELECT * FROM hcl_nomeclador WHERE idprofesional=$idprofesional");				
				$pro = new clase_listar();			
	      ...
	      function foranea_idpaciente($idpaciente)
		  {
				$bd = new baseDatos();
				$bd->Conectarse();		    
				$bd->select("SELECT * FROM hcl_nomeclador WHERE idpaciente=$idpaciente");				
				$pro = new clase_listar();			
