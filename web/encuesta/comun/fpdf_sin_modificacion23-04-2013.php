...
function AddPage($orientation='',$numero_pagina=0)
{
	//Start a new page
	...
	$this->TextColor=$tc;
	$this->ColorFlag=$cf;
	//codigo agregado por mi para poder poner numeo de pag a las hojas horizontales
	if ($orientation == 'L')
	{
	    $this->HeaderHorizontal($numero_pagina);
	}
	else
	{
	    //Page header
	    $this->Header($numero_pagina);
    }		
	//Restore line width
...
function HeaderHorizontal($numero_pagina)
{
	//To be implemented in your own inherited class
	$this->SetFont('arial','B',10);
	 $this->Cell(250);
	if ($numero_pagina == 1)  	
	    $this->Cell(5,8,'pag.'.$this->PageNo(),0,1);//esto lo puse yo
	
}
function Header($numero_pagina)
{
	//To be implemented in your own inherited class
	$this->SetFont('arial','B',10);
	 $this->Cell(160);
...
function ImprovedTable($header,$data)
{
    ...
    foreach($data as $row)
    {
        ...
        $this->MultiCell($w[1],5,$row[1],1);
        $this->Cell($w[2],5,$row[2],1);
        $this->Ln();
    }
    //Línea de cierre
	//T = superior
