...
return [
    ...
    [
        ...
        'label' => 'Solicitante',
        'value' => function ($model) {
            $idcontacto = $model->usuario_solicitante;
            if ($idcontacto != null) {
        ...
        'filter' => ArrayHelper::map(
            Sds_com_persona::findBySql(
                ...
                (SELECT usuario_solicitante FROM sds_reg_registro AS reg INNER JOIN sds_reg_tipo AS tipo ON tipo.idtipo=reg.idtipo 
                    WHERE tipo.entidad=".Sds_reg_registro::ENT_MANTENIMIENTO.")) 
    ...
    [
        ...
        'filterType' => GridView::FILTER_SELECT2,
        'filter' => ArrayHelper::map(Sds_reg_tipo::find()->where(['entidad'=> Sds_reg_registro::ENT_MANTENIMIENTO])->orderBy(['descripcion' => SORT_ASC])->all(), 'idtipo', 'descripcion'),
        'filterWidgetOptions' => [
            'pluginOptions' => ['allowClear' => true],
        ],
    ...
    [
        ...
        'template' => ' {view} {update} {imprimir} {delete}' .
            ($permiso_logs ? ' {logs}' : ''),
        'dropdown' => false,
        'vAlign' => 'middle',
