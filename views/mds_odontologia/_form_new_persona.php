            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.status === 'success' && data?.records?.idpersona) {
                        ...
                        $('input[name="Mds_odontologia[idpersona]"]').trigger("input");
                        $("#txtDNI_search").val(`${record.apellido} ${record.nombre} (${record.documento})`)
                        $("#mds_odontologia-idpersona").val(record.idpersona);
                        $('.modal.in').modal('hide')
                    } else {
                        $("#txt_mensaje").html("<b>Error!</b><i>No se ha podido cargar la persona</i>");
                    }
                },
