...
<div class="mds-cap-persona-form">
    ...
    <div class="row">
        <div class="col-md-5 col-md-offset-5" style="padding-top:30px;" id="txt_mensaje">
        </div>
    </div>
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.status === 'success' && data?.records?.idpersona) {
                        alert("Se ha cargado correctamente a la persona");
                        const record = data.records;
                        $('input[name="mds_gerontologia[idpersona]"]').val(record.idpersona)
                        $('input[name="mds_gerontologia[idpersona]"]').trigger("input");
                        $("#txtDNI_search").val(`${record.apellido} ${record.nombre} (${record.documento})`)
                        $("#mds_gerontologia-idpersona").val(record.idpersona);
                        $('.modal.in').modal('hide')
                        $('#campos').show();
                        $('#divDatepicker').show();
                        $('#btnSave').show();
                    } else {
                        $("#txt_mensaje").html("<b>Error!</b><i>No se ha podido cargar a la persona</i>");
                        $('#campos').hide();
                        $('#divDatepicker').show();
                        $('#btnSave').hide();
                    }
