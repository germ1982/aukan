...
$this->title = "Respuestas Requerimiento #{$oficio->idlegalesoficio}";
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body" style="word-break: break-all" ;>
                            ...
                            <div class="alert alert-respuesta" role="alert">
                                ...
                                <?php if (!$respuesta->yaTieneEstado()) : ?>
                                    <?php if ($htmlAdjuntosSupervisor) : ?>
                                        <?= $htmlAdjuntosSupervisor ?>
                                        <br />
                                    <?php endif ?>
                                    <?php if ($hasRolSupervisor) : ?>
                                        <hr>
                                        <div style="margin-bottom: 1rem">
                                            <button data-toggle="modal" href="#mi_modal" data-estado="observar" data-titulomodal="Devolver a generador/a de respuestas" data-idlegalesrespuesta="<?php echo $respuesta->idlegalesrespuesta; ?>" class="btn btn-danger btn-estado">Devolver a generador/a de respuestas</button>
                                            <button data-toggle="modal" href="#mi_modal" data-estado="aprobar" data-titulomodal="Aprobar respuesta" data-idlegalesrespuesta="<?php echo $respuesta->idlegalesrespuesta; ?>" class="btn btn-success btn-estado">Aprobar</button>

                                            <?php
                                            $mostrarBotonVisto = true;
                                            $respuestaVistosLength = count($respuesta->vistos);
                                            $indexRespuestaVisto = 0;
                                            while ($indexRespuestaVisto < $respuestaVistosLength && $mostrarBotonVisto) {
                                                $respuestaVisto = $respuesta->vistos[$indexRespuestaVisto];
                                                if ($respuestaVisto['idusuario'] == $idusuario) {
                                                    $mostrarBotonVisto = false;
                                                }
                                                $indexRespuestaVisto++;
                                            }

                                            if ($mostrarBotonVisto) :
                                            ?>
                                                <button data-toggle="modal" href="#modal_visto" id="BOTON_VISTO" data-idlegalesrespuesta="<?php echo $respuesta->idlegalesrespuesta; ?>" class="btn btn-info">Marcar visto</button>
                                            <?php endif; ?>
                                        </div>
                                    <?php endif ?>
                            ...
                            <?php if ($respuesta->ultimoEstado->estado == $idEstadoRechazado) : ?>
                                <div class="alert alert-danger" role="alert">
                                    ...
                                    <br />
                                    <?php if ($respuesta->derivacion && Mds_legales_oficio::tieneRol(Mds_legales_oficio::ID_ROL_SUPERVISOR)) : ?>
                                        <div class="card-footer" id="botones">
                                            <button data-toggle="modal" href="#mi_modal" data-estado="observar" data-titulomodal="Devolver a generador/a de respuestas" data-idlegalesrespuesta="<?php echo  $respuesta->idlegalesrespuesta; ?>" class="btn btn-danger btn-estado">Devolver a generador/a de respuestas</button>
                                            <!-- <button data-toggle="modal" href="#mi_modal" data-estado="aprobar" data-titulomodal="Aprobar respuesta" data-idlegalesrespuesta="<? //php echo $respuesta->idlegalesrespuesta; 
                                                                                                                                                                                    ?>" class="btn btn-success btn-estado">Aprobar</button> -->
                                        </div>
                                    <?php endif ?>
                                </div>
                            <?php endif ?>
                            ...
                            <?php if ($respuesta->ultimoEstado->estado == $idEstadoObservada) { ?>
                                <div class="alert alert-danger" style="margin-bottom: 0;" role="alert">
                                    ...
                                    <?php endif ?>
                                    <br />
                                    <?php if ($respuesta->derivacion && ($respuesta->idusuario == $idusuario) && !$respuesta['idrespuestacorreccion']) : ?>
                                        <div class="card-footer">
                                            <?= Html::a('Modificar ', ['mds_legales_respuesta/create', 'idDerivacion' => $respuesta->derivacion->idlegalesderivacion, 'idrespuesta' => $respuesta->idlegalesrespuesta], ['class' => 'btn btn-success']); ?>
                                        </div>
                                    <?php endif; ?>
                                </div>
                            <?php } ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row" style="padding:15px">
                    <p>Observación:</p>
                    <?php $form = ActiveForm::begin(['id' => 'form-response', 'action' => ['mds_legales_respuesta_estado/store']]); ?>
                    <input type="hidden" name="estado" id="hfEstado" value="">
                    <input type="hidden" id="idlegalesrespuesta" name="idlegalesrespuesta" value="">
            ...
            <div class="modal-footer">
                <?= Html::submitButton("Guardar", ['class' => 'btn btn-success pull-left', 'id' => 'btnGuardar']) ?>
...
<div class="modal fade" id="modal_visto" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?php $form = ActiveForm::begin(['id' => 'form-response', 'action' => ['mds_legales_respuesta_visto/store']]); ?>
                <input type="hidden" id="idlegalesrespuesta_visto" name="idlegalesrespuesta_visto" value="">
                <div class="row" style="padding:15px">
                    ¿Está seguro que desea marcar como <b>"visto"</b> esta respuesta?
                </div>
            </div>
            <div class="modal-footer">
...
$this->registerJs(
    "$('.btn-estado').click(function(){
           ...
           let tituloModal = $(this).data('titulomodal');
           $('#titulo-modal').html(tituloModal);
           $('#hfEstado').val(estado);
           let respuestaid = $(this).data('idlegalesrespuesta');
           $('#idlegalesrespuesta').val(respuestaid);
           if (estado==='aprobar'){
                $('#observaciones').html('Aprobado');
           } else {
                $('#observaciones').html('');
           }
      });
    ...
    $('#BOTON_VISTO').click(function(){
           const respuestaid = $(this).data('idlegalesrespuesta');
           $('#idlegalesrespuesta_visto').val(respuestaid);
      });
      
      $('#btn-ver-comprobante').click(function(e){
        e.stopPropagation();
        });
      $('#btn-ver-nota').click(function(e){
        e.stopPropagation();
        });
